...
friendab = 'frindab.data'

if os.path.exists('friendab.data')==False:
    Dumpfile(ab)
    del ab
else:
    pass
...
class Person:
    def __init__(self, name):
        self.name = name
    def saysome(self):
        print 'The friend %s, his email is %s' %(sname, frilist[sname])
...
[3]: Del your friend's email from friendbook
[4]: Alter your friend's email from friendbook
[5]: All friends list
[6]: exit the program
'''
while True:
    f = open('friendab.data')
    frilist = pickle.load(f)
    f.close()
